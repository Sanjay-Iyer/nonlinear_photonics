...
def build_inputs(kp8_root, singleband_root):
    # Demo 30 change (see PROVENANCE.md): the 8-band and single-band runs are separate
    # registered packages, so both folders are passed explicitly instead of one root
    # with fixed 'kp8' / 'singleband_case04_graded' subfolders. Nothing else differs.
    sb=io.read_single_band(Path(singleband_root))
