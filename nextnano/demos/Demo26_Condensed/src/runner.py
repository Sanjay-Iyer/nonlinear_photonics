...
METADATA = "demo26_run_metadata.json"
EXPECTED = {
...
def build_jobs(config: dict, which: str, overrides: dict) -> list[dict]:
    ...
    jobs = [{"job": "kp8", "kind": "kp8", "purpose": config["kp8"]["purpose"], "params": params,
             "text": decks.render_kp8(params, "Demo26_Condensed production kp8 deck")}]
    if which == "all":
...
def parser_probe(root: Path) -> dict:
    ...
    try:
        from .raw_inputs import available_modes
        from .kp8_states import load_kp8_inputs
        modes = available_modes(root)
        result = {"modes": modes}
        if modes.get("kp8") == "available":
            result["kp8_state_ids"] = load_kp8_inputs(Path(root) / "kp8")["state_ids"]
        return result
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(prog="01_run_nextnano.py", description=__doc__)
    ...
    ap.add_argument("--output", type=Path, default=Path(os.environ.get("DEMO26_RAW_OUTPUT", ROOT / "output" / "raw")))
    ...
    try:
        ...
        for job in jobs:
            ...
            if not args.no_parse and parser_exe and parser_db:
                with tempfile.TemporaryDirectory(prefix="demo26_parse_") as tmp:
        ...
        manifest = {"created_utc": dt.datetime.now(dt.timezone.utc).isoformat(), "mode": run_mode,
                    ...
                    "script2_parser_self_test_on_shipped_cached_raw": parser_probe(ROOT / "cached_raw"),
        ...
        save()
        print(f"All jobs PASS. Next: python 02_calculate_chi2.py --input \"{output}\"")
        return 0
