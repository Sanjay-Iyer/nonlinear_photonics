...
def build_jobs(config: dict, which: str, overrides: dict) -> list[dict]:
    ...
    jobs = [{"job": "kp8", "kind": "kp8", "purpose": config["kp8"]["purpose"], "params": params,
             "text": decks.render_kp8(params, "Demo 28 production kp8 deck")}]
    if which == "all":
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--output", type=Path, default=Path(os.environ.get("DEMO28_RAW_OUTPUT", ROOT / "output" / "raw")))
