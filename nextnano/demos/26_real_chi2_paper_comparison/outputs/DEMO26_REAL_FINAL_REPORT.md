# Demo 26_real final report

## 1. Executive summary

The paper does **not** match normalized signed Re[chi2] better than normalized |chi2|. On the unchanged 400-1850 nm overlap, signed Re has normalized RMSE 0.3834, versus 0.2624 for |chi2|. Signed Re crosses zero at 714.03 and 1437.48 nm, not near the paper's 605 and 1330 nm zero-valued features. The paper itself explicitly labels Figure 2d as simulated |chi2|, so the observable-reinterpretation hypothesis is rejected.
...
Signed Re[chi2] is read directly from the preserved Demo 23D complex spectrum. Its range is -65.8903 to 71.3106 pm/V. The normalized signed trace is shown in Figures 1, 2, 5, 6, and 7.

## 7. Im[chi] calculation

Im[chi2] is read from the same complex spectrum and ranges from -57.8979 to 29.0566 pm/V. Figure 3 shows it separately so the main comparison is not cluttered.

## 8. |chi| calculation

The magnitude is the stored and independently verified `hypot(Re, Im)`, ranging from 0.3999 to 81.4907 pm/V. Figure 4 shows the absolute spectrum.
...
Figure 1 is the decisive overlay. |chi2| has the lower RMSE (0.2624 vs 0.3834). Correlations are 0.3643 for signed Re and 0.2476 for |chi2|; the higher Re correlation does not overcome its larger amplitude error, incorrect sign over broad intervals, missing features, or conflict with the paper label. Figure 9 shows the residuals.

## 11. 605-nm sign-flip analysis

At the paper wavelength, Re = 3.311856, Im = 0.061125, and |chi2| = 3.312420 pm/V. The nearest Re crossing is 714.029 nm, bracketed by Re = 0.015486 at 714 nm and -0.525179 at 715 nm. Im and |chi2| at that interpolated crossing are 27.278973 and 27.279122 pm/V. Re changes sign at the crossing, but **not near 605 nm**.

## 12. 1330-nm sign-flip analysis

At the paper wavelength, Re = -25.208647, Im = -1.031832, and |chi2| = 25.229756 pm/V. The nearest Re crossing is 1437.480 nm, bracketed by Re = -0.230866 at 1437 nm and 0.249990 at 1438 nm. Im and |chi2| at the crossing are -52.996929 and 52.997473 pm/V. Re changes sign at the crossing, but **not near 1330 nm**.

## 13. Peak correspondence

- P1: paper 540 nm; signed Re N/A nm; |chi| N/A nm; better: neither: no corresponding feature.
- P2: paper 760 nm; signed Re 753.0 nm; |chi| 752.0 nm; better: signed Re[chi2].
- P3: paper 1080 nm; signed Re N/A nm; |chi| N/A nm; better: neither: no corresponding feature.
- P4: paper 1520 nm; signed Re 1505.0 nm; |chi| 1503.0 nm; better: signed Re[chi2].
...
| comparison | normalized_RMSE | correlation_coefficient | normalized_MAE | matched_peak_count | peak_wavelength_MAE_nm | zero_or_minimum_feature_MAE_nm |
|---|---|---|---|---|---|---|
| paper_vs_signed_Re_chi2 | 0.3834 | 0.3643 | 0.3050 | 2 | 11.0000 | 108.2544 |
| paper_vs_abs_chi2 | 0.2624 | 0.2476 | 0.2066 | 2 | 12.5000 | 120.0000 |
...
| Demo24_conclusion | classification | Demo26_real_evidence |
|---|---|---|
| missing P1/P3 | STILL VALID | Neither signed Re nor |chi| has a significant corresponding extremum in the configured P1/P3 windows; changing observable does not create the missing peak pair. |
| missing 605 node | STILL VALID | Nearest Re zero is about 714 nm, outside 550-650 nm; |chi| at 605 nm is finite. |
| missing 1330 node | STILL VALID | Nearest Re zero is about 1437 nm, outside 1250-1400 nm; |chi| at 1330 nm is finite. |
| 2.296 eV missing transition | STILL VALID | The P1/P3 one-/two-photon pair remains absent in both observables; taking Re cannot supply a missing denominator resonance. |
| kmax artifact | UNRELATED TO REAL-vs-MAGNITUDE ISSUE | Changing the displayed complex component cannot cure a feature that moves with the integration cutoff. |
| finite-k M(k) required | WEAKENED | Finite-k matrices remain required for the next discriminating test of cancellation and high-k damping, but existing perturbations do not prove that physical M(k) will cure the missing features. |
| additional states required | WEAKENED | The missing 2.296 eV resonance remains, but the paper explicitly states only two bound states per band. Extra optical-sum states would change the stated model; structure/cutoff/state-tracking must be isolated first. |
| geometry mismatch | UNRELATED TO REAL-vs-MAGNITUDE ISSUE | The published design/abrupt-interface versus Demo 23 linear-1-nm mismatch is independent of whether Re or magnitude is plotted. |
| need for Demo 25 Professional run | STILL VALID | The observable reinterpretation fails the direct zero-crossing test and gives worse overall normalized RMSE, so unresolved finite-k/model-consistency questions remain. |
