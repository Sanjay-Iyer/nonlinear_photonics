...
| 08 | what happens to the wavefunctions | D |
...
| `06_envelope_demonstration.csv` | real licensed envelopes (see §6 below) |
...
The one thing that genuinely is *not* in this repository is case 04's own
**envelope functions** ψ(z) — those live beside the raw licensed run on the
work laptop. STEP 08 says so explicitly rather than inventing a curve. It then
demonstrates the envelope → matrix-element step on the one licensed envelope
table that *is* present (Demo 11's `s1_ref`, gitignored, **a different
structure**), showing that recomputing O, z^e and z^hh from those envelopes
reproduces that run's own `matrix_elements.json` to ~1e-17. Those numbers are
labelled as belonging to a different structure and are used nowhere else.
