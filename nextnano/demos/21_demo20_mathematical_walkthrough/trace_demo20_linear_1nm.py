...
#: A real licensed envelope table that exists in this checkout. It belongs to
#: Demo 11's s1_ref case - a DIFFERENT structure from Demo 20 case 04 - and is
#: used in STEP 08 only to demonstrate the envelope -> matrix-element step
#: numerically. Its numbers are never mixed into case 04's calculation.
...
""".strip(NL))
    block("WHY NO ENVELOPE ARRAY IS PRINTED FOR CASE 04", """
The envelopes psi_e1..psi_hh2 for case 04 live only beside the raw licensed
run, on the work laptop (spectrum_path in the results table points at
C:\\nn_results\\...). They are not in this repository. The results table carries
the QUANTITIES DERIVED FROM THEM - O, z^e, z^hh - which is precisely what the
susceptibility needs, so nothing downstream is missing. This trace does not
invent an envelope to fill the gap.
""".strip(NL))

    aux = _auxiliary_envelope_demo(out("06_envelope_demonstration.csv"))
    if aux is None:
        block("NUMERICAL DEMONSTRATION", [
            f"skipped: no licensed envelope table found at {AUX_ENVELOPES}",
            "(that directory is gitignored, so it exists only on a machine",
            " that has run Demo 11.)",
        ])
    block("MEANING", """
...
they are multiplied together and integrated. Two numbers per pair.
""".strip(NL))
    block("CHANGED",
...
def _auxiliary_envelope_demo(path: Path | None) -> dict[str, Any] | None:
    """Demonstrate the envelope -> matrix-element step on real licensed envelopes.

    IMPORTANT: this uses Demo 11's ``s1_ref`` run, which is a DIFFERENT
    structure from Demo 20 case 04. Its numbers are NOT case 04's and are never
    mixed into the trace. It is here because it is the only licensed envelope
    table present in this checkout, and it lets STEP 09/10's integrals be shown
    working on actual wavefunctions: recomputing them from the envelopes must
    reproduce that run's own recorded matrix_elements.json.
    """

    envelopes_csv = AUX_ENVELOPES / "envelopes.csv"
    recorded_json = AUX_ENVELOPES / "matrix_elements.json"
    if not (envelopes_csv.is_file() and recorded_json.is_file()):
        return None

    data = np.loadtxt(envelopes_csv, delimiter=",", skiprows=1)
    z = data[:, 0]
    psi_e = data[:, 1:3]
    psi_h = data[:, 3:5]
    # Energies are placeholders here: BandStates needs one per envelope, but
    # only the ENVELOPES are used by overlap_matrix / position_matrix.
    band_e = shared_chi2.BandStates(z, np.array([0.0, 1.0]), psi_e, "e")
    band_h = shared_chi2.BandStates(z, np.array([0.0, -1.0]), psi_h, "hh")
    ...
    deviations = {
        "overlap": float(np.max(np.abs(
            overlap - np.asarray(recorded["overlap_electron_hole"])))),
        "z_electron": float(np.max(np.abs(
            z_e - np.asarray(recorded["position_matrix_electron_nm"])))),
        "z_heavy_hole": float(np.max(np.abs(
            z_h - np.asarray(recorded["position_matrix_heavy_hole_nm"])))),
    }
    assert max(deviations.values()) <= 1.0e-13, deviations

    block("NUMERICAL DEMONSTRATION ON REAL LICENSED ENVELOPES", f"""
Source: {AUX_ENVELOPES.relative_to(REPO_ROOT)}
        (Demo 11 case s1_ref - a DIFFERENT STRUCTURE from Demo 20 case 04.
         These numbers are NOT case 04's and are used nowhere else in this
         trace. They are here only to show the integrals working on actual
         wavefunctions, because case 04's own envelopes are not in this repo.)

Envelope table: {psi_e.shape[0]} grid points, z from {z[0]:.4f} to {z[-1]:.4f} nm
Columns: z_nm, psi_e1, psi_e2, psi_hh1, psi_hh2

    psi_e1 first 4 values : {np.array2string(psi_e[:4, 0], precision=6)}
    psi_e1 max            : {np.max(np.abs(psi_e[:, 0])):.6f} nm^-1/2
    int psi_e1^2 dz after normalization : \
{float(np.trapezoid(band_e.envelopes[:, 0]**2, z)):.15f}

Orthonormality (max |<psi_i|psi_j> - delta_ij|):
    electron band   : {band_e.orthonormality_error():.3e}
    heavy-hole band : {band_h.orthonormality_error():.3e}

Recomputing the matrix elements from these envelopes with
_shared/chi2.overlap_matrix and position_matrix reproduces the run's own
matrix_elements.json to:
    O      max |diff| = {deviations['overlap']:.3e}
    z^e    max |diff| = {deviations['z_electron']:.3e}
    z^hh   max |diff| = {deviations['z_heavy_hole']:.3e}

=> The equations in STEP 09 and STEP 10 are exactly the ones that produced
   case 04's O, z^e and z^hh during its licensed run.
""".strip(NL))

    if path is not None:
        write_csv(path,
                  ["z_nm", "psi_e1", "psi_e2", "psi_hh1", "psi_hh2",
                   "psi_e1_psi_hh1", "psi_e1_z_psi_e1"],
                  [[f"{z[i]:.17g}",
                    f"{band_e.envelopes[i,0]:.17g}", f"{band_e.envelopes[i,1]:.17g}",
                    f"{band_h.envelopes[i,0]:.17g}", f"{band_h.envelopes[i,1]:.17g}",
                    f"{band_e.envelopes[i,0]*band_h.envelopes[i,0]:.17g}",
                    f"{band_e.envelopes[i,0]*z[i]*band_e.envelopes[i,0]:.17g}"]
                   for i in range(z.size)])
    return {"deviations": deviations, "source": str(AUX_ENVELOPES)}
