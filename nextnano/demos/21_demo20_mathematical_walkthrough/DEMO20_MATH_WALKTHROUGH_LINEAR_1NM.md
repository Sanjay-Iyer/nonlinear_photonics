...
They live only beside the raw licensed run, on the work laptop —
`spectrum_path` in the results table points at `C:\nn_results\...`. They are not
in this repository. The results table carries the *quantities derived from them*
(O, z^e, z^hh), which is exactly what the susceptibility needs, so nothing
downstream is missing. **This walkthrough does not invent an envelope to fill
the gap.**

### Numerical demonstration on real licensed envelopes

There *is* one licensed envelope table in this checkout — Demo 11's `s1_ref`
case at
`nextnano/results/demo_runs/11_paper_validation_interband_chi2_acqw/20260731T002541Z_62efb35c/runs/s1_ref/extracted/`
(gitignored, so present only on a machine that has run Demo 11).

**It is a different structure from Demo 20 case 04 and its numbers are used
nowhere else in this walkthrough.** It is shown because it lets §12 and §13's
integrals be watched working on actual wavefunctions:

```
Envelope table : 55 grid points, z from 3.5253 to 26.4747 nm
Columns        : z_nm, psi_e1, psi_e2, psi_hh1, psi_hh2
psi_e1 first 4 : [0.  0.000626  0.001433  0.002647]
psi_e1 max     : 0.445780 nm^-1/2
∫ psi_e1² dz after normalization : 1.000000000000000

Orthonormality (max |<psi_i|psi_j> − δ_ij|)
    electron band   : 3.918e-15
    heavy-hole band : 1.255e-14

Recomputing O, z^e, z^hh from these envelopes with
_shared/chi2.overlap_matrix and position_matrix reproduces that run's own
matrix_elements.json to
    O     max|diff| = 1.735e-17
    z^e   max|diff| = 0.000e+00
    z^hh  max|diff| = 3.553e-15
```

⇒ The equations in §12 and §13 are exactly the ones that produced case 04's
O, z^e and z^hh during its licensed run.
...
Relative to the abrupt reference (case 00, 31.036041396587407 pm/V raw):
**0.5815** — 1.0 nm of linear grading costs 42 % of the response.
